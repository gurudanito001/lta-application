...
interface getCallsForTwoUsers {
  user1: string,
  user2: string,
  order?: string,
  status?: string //"missed" | "answered" | "10mins or less" | "30mins or less" | "more than 30mins"
}
export const getAllCallsBetweenTwoUsers = async(filters: getCallsForTwoUsers,  pagination: {page: string, take: string}) => {
  //await prisma.call.deleteMany();
  function deriveDurationFilter (status?: string ){
    switch(status){
      case "missed":
  ...
  const calls = await prisma.call.findMany({
    where: {
      OR: [
        { 
          AND: {
            callerId: filters?.user1,
            calleeId: filters?.user2
          }
        },
        {
          AND: {
            callerId: filters?.user2,
            calleeId: filters?.user1
          }
        }
      ],
      duration: { 
